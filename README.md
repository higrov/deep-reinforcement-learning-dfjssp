...
- `--record` will save the observation at each time step as an image in `recordings`.
...
To run our full suite of computational experiments (self-play and ad-hoc), we've provided the script `run_experiments.sh` that runs our experiments on 20 seeds with `2` agents.
