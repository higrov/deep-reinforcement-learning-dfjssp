...
class STRIPSWorld:
    ...
    def get_subtasks(self, recipe__, max_path_length=10, draw_graph=False):
        action_paths = []

        graph, goal_state = self.generate_graph(recipe__, max_path_length)
        ...
        for state_path in all_state_paths:
            for i in range(len(state_path) - 1):
                union_action_path.add(graph[state_path[i]][state_path[i + 1]]["obj"])
            # action_path = [
